...
特定の駅に到着する列車を到着時刻で昇順に取得します。オプションで `railway` / `calendar` / `direction` による絞り込みが可能です。

- **station** (string): 駅のURI（必須）
- **railway** (string|null): 路線URI（任意。指定するとその路線の列車に絞る）
- **calendar** (string|null): カレンダー種別URI（任意）
- **direction** (string|null): 方向URI（任意）
- **戻り値**: 以下の構造を持つオブジェクト配列（`arrivalTime` で昇順）
  ```javascript
  {
    railway: string,        // 線路URI
    calendar: string,       // カレンダータイプURI
    ...
    arrivalTime: string,    // 到着時間（HH:MM:SS形式）
    stopIndex: number       // 停車駅インデックス（0始まり）
  }
  ```

該当する駅や条件がない場合は空配列を返します。
